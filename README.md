...
> 基于 LangChain、FAISS、BM25 和 RRF 构建面向英文机器学习 Markdown 笔记的中文问答 RAG 系统；实现标题感知分块、主题元数据过滤、多语向量检索和中文查询改写，并支持基于检索来源的中文回答生成。

## 上传 GitHub 注意事项

建议只上传源码、README、requirements 和必要的小型示例文件。不要上传本地环境、缓存、密钥或可重新生成的大文件。
...
关于 `data/ML-Notes-in-Markdown-master`：

- 如果这是第三方数据集，先检查其 LICENSE 是否允许再分发。
- 如果不确定，建议不要直接提交完整数据；可以在 README 中说明数据来源和放置路径。
- 如果要提交数据，保留原始 LICENSE，并在 README 中明确注明来源。

关于索引文件：

- `code/vector_index_ml_notes/index.faiss` 和 `index.pkl` 是可由脚本重新生成的产物。
- 不建议提交到 GitHub，除非你明确希望别人 clone 后免构建直接运行。
- 如果提交索引，需要确认文件大小不会让仓库变得臃肿。

关于 API key：

- 不要把 `MOONSHOT_API_KEY` 写进代码或 README 示例真实值。
- 推荐用户通过环境变量设置：
