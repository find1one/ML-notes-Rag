...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare PC0 raw children with PC1 contextual children.")
...
def evaluate_ablation_cases(retriever, cases: Sequence, top_k: int) -> List[dict]:
    """Evaluate PC0/PC1 ranks without changing the shared evaluation path."""
    results = []
...
def compare_case_reports(baseline: Sequence[dict], candidate: Sequence[dict]) -> List[dict]:
    changes = []
...
def main() -> int:
    ...
    pc1_vectorstore = index_module.build_vector_index(contextual_children, write_manifest=False)
    pc1_child_retriever = ExperimentChildRetriever(pc1_vectorstore, contextual_children)
    pc1_retriever = ParentChildRetriever(pc1_child_retriever, parent_by_id, len(contextual_children))
    pc1_results = evaluate_ablation_cases(pc1_retriever, EVAL_CASES, args.top_k)

    pc0_evaluation = _evaluation_summary(pc0_results, args.top_k)
    pc1_evaluation = _evaluation_summary(pc1_results, args.top_k)
    pc0_cases = _case_report(pc0_results)
    pc1_cases = _case_report(pc1_results)
    changes = compare_case_reports(pc0_cases, pc1_cases)
    source_rank_change_count = sum(
        change["evaluable"]
        and change["pc0"]["source_rank"] != change["pc1"]["source_rank"]
    ...
    report = {
        ...
        "candidate_k": 80,
        "pc0_evaluation": pc0_evaluation,
        "pc1_evaluation": pc1_evaluation,
        "pc0_retrieval_stats": _retrieval_stats(pc0_results),
        "pc1_retrieval_stats": _retrieval_stats(pc1_results),
    ...
    print("Parent-child PC0/PC1 ablation evaluation")
    ...
    for label, evaluation in (("PC0", pc0_evaluation), ("PC1", pc1_evaluation)):
    ...
    if changes:
        ...
        for change in changes:
            ...
            print(
                "      "
                f"source rank {pc0['source_rank']} -> {pc1['source_rank']}; "
                f"topic rank {pc0['topic_rank']} -> {pc1['topic_rank']}"
            )
            print(f"      PC0 paths: {pc0['paths']}")
            print(f"      PC1 paths: {pc1['paths']}")
